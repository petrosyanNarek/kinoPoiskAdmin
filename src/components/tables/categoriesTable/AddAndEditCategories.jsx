...
import { getCategoryById, newCategories, updateCategories, selectCategoriesLoading, selectCategoriesError } from './../../../features/categoriesSlice';
...
export const AddAndEditCategories = () => {
    ...
    const { register, setValue, handleSubmit, formState: { errors, touchedFields }, reset } = useForm({
    ...
    useEffect(() => {
        if (categoriesId) {
            dispatch(getCategoryById(categoriesId)).unwrap().then(r => {
                ...
                categoriesLoading
                    ...
                    categoriesError
                        ...
                        <div className="new-film">
                            <h2 className="text-center">{categoriesId ? "Edit Category" : "Add Category"}</h2>
                            <form onSubmit={handleSubmit(onSubmitHandler)}>
                                <div className="group">
                                    <div className="w-75  text-primary">
                                        <label htmlFor="" className=" fw-bold m-3">Name</label>
                                        <input {...register('name')} placeholder="Name..." name="name" className='input' />
                                        {touchedFields.name && errors.name?.message && <div className="errors">{errors.name?.message}</div>}
                                    </div>
                                </div>
                                <div className="group mt-5">
                                    <div className="w-75 text-center text-primary">
                                        <button type="submit" className="w-100 fw-bold rounded-pill btn btn-primery serach-btn">
                                            Submit
                                        </button>
                                    </div>
                                </div>
